<template>
  <!--第一个：class，主要是为了在点击提问给body加一个灰色的半透明蒙层，蒙层用的类名---AFTER----的after属性控制，-->
  <div :class="{'simpleMain':IsTrue,'AFTER':FuTrue}">
    <div class="simpleMain-header">
      ...
      <div class="simpleMain-header-shadom">
        <p> {{this.res.goods_name}}</p>
      </div>
      <p class="simpleMain-header-p">
        {{this.res.goods_content}}
      </p>
      <p class="simpleMain-header-pA">
        <span class="simpleMain-header-imgA"><img src="../../static/images/simpleMain/订单.png" /> {{this.res.sales_sum}}测试</span>
        <span class="simpleMain-header-imgB"><img src="../../static/images/simpleMain/待评价.png"/> {{this.res.comment_count}}评价</span>
      </p>
    </div>
    <p class="simpleMain-commit-p">评论反馈（{{this.res.comment_count}}）</p>
    <div
    ...
    <p class="simpleMain-tent-pap"><router-link to="/simple-main/allcommit">全部评论{{this.res.comment_count}}条></router-link></p>
    <nav-simple-water></nav-simple-water>
    <div class="simpleMain-submit">
      <p class="simpleMain-submitA">￥ {{this.res.shop_price}}</p>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      this.Nub = this.$route.query.list//而这里却成功赋值
      this.res = res.data.data.hotGoods[ this.Nub]
