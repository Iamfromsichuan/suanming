...
<script>
  ...
  export default {
    ...
    created(){
      var list =[]
      var masterQingceOrder=[]
      var userwaitToCommit=[]
      var userwaitingPayRes=[]
      for(let i =0;i<JSON.parse(sessionStorage.getItem("masterQingceOrder")).orders.length;i++ ){
        masterQingceOrder.push(JSON.parse(sessionStorage.getItem("masterQingceOrder")).orders[i])
      }
